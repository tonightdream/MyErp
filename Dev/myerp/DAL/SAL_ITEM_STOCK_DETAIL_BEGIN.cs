...
{
	/// <summary>
	/// 数据访问类:SAL_ITEM_STOCK_DETAIL_BEGIN
	/// </summary>
	public partial class SAL_ITEM_STOCK_DETAIL_BEGIN
	{
		...
		public bool Delete()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from SAL_ITEM_STOCK_DETAIL_BEGIN ");
			strSql.Append(" where ");
			SqlParameter[] parameters = {
			};
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM SAL_ITEM_STOCK_DETAIL_BEGIN ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM SAL_ITEM_STOCK_DETAIL_BEGIN ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from SAL_ITEM_STOCK_DETAIL_BEGIN T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "SAL_ITEM_STOCK_DETAIL_BEGIN";
