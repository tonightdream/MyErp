...
{
	/// <summary>
	/// 数据访问类:ORDER_SEND_DETAIL
	/// </summary>
	public partial class ORDER_SEND_DETAIL
	{
		public ORDER_SEND_DETAIL()
		{}
		#region  BasicMethod
		...
		public bool Delete()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from ORDER_SEND_DETAIL ");
			strSql.Append(" where ");
			SqlParameter[] parameters = {
			};
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select DETAILID,MASTERID,BILLNO,Sequence,ITEM_INTERNAL_CODE,ITEM_CODE,ITEM_NAME,ITEM_COLOR,PARENT_ITEM_CODE,ITEM_TYPE,ITEM_CD,ITEM_KD,ITEM_GD,ITEM_SPECIFICATIONS,ITEM_MEASURE_UNIT,ITEM_GYLC,ITEM_POSITION,ITEM_MIN_QTY,ITEM_MAX_QTY,ITEM_IMAGE,ITEM_IMAGE_NAME,ITEM_DWMJ,ITEM_BARCODE,NET_PRICE,PRICE,Quantity,ORD_CGqty,ORD_CLqty,ORD_PCqty,ORD_WGqty,ORD_TCqty,ORD_WGZL,ORD_FPqty,ORD_Sendqty,ORD_SHqty,ORD_LHqty,ORD_HZqty,ORD_THqty,ORD_Acceptqty,ORD_TL,ORD_GRAPHICNO,ORD_SHDH,REMARK ");
			strSql.Append(" FROM ORDER_SEND_DETAIL ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM ORDER_SEND_DETAIL ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from ORDER_SEND_DETAIL T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "ORDER_SEND_DETAIL";
