...
namespace MyERP.DAL
{
	...
	public partial class ORDER_Capacity_chart
	{
		...
		public bool Delete()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from ORDER_Capacity_chart ");
			strSql.Append(" where ");
			SqlParameter[] parameters = {
			};
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select INTERNAL_CODE,BILLDATE,DEPARTMENT_CODE,DEPARTMENT_NAME,QUANTITY ");
			strSql.Append(" FROM ORDER_Capacity_chart ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM ORDER_Capacity_chart ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from ORDER_Capacity_chart T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "ORDER_Capacity_chart";
