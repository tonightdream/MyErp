...
{
	/// <summary>
	/// 数据访问类:PUB_ITEM_DA_BOM
	/// </summary>
	public partial class PUB_ITEM_DA_BOM
	{
		public PUB_ITEM_DA_BOM()
		{}
		#region  BasicMethod
		...
		/// 增加一条数据
		/// </summary>
		public bool Add(MyERP.Model.PUB_ITEM_DA_BOM model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("insert into PUB_ITEM_DA_BOM(");
		...
		/// 更新一条数据
		/// </summary>
		public bool Update(MyERP.Model.PUB_ITEM_DA_BOM model)
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("update PUB_ITEM_DA_BOM set ");
		...
		public bool Delete()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("delete from PUB_ITEM_DA_BOM ");
			strSql.Append(" where ");
			SqlParameter[] parameters = {
			};
		...
		/// 得到一个对象实体
		/// </summary>
		public MyERP.Model.PUB_ITEM_DA_BOM GetModel()
		{
			//该表无主键信息，请自定义主键/条件字段
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select  top 1 SEQUENCE,ITEM_INTERNAL_CODE,ITEM_CODE,ITEM_NAME,ITEM_COLOR,PARENT_ITEM_CODE,ITEM_TYPE,SPECIFICATIONS,MEASURE_UNIT,ITEM_CZ,ITEM_LH,ITEM_LK,ITEM_BJ,ITEM_JZ,ITEM_HZ,ITEM_rate,ITEM_GYLC,NET_PRICE,PRICE,MIN_QTY,MAX_QTY,POSITION,IMAGE,IMAGE_NAME,Quantity,BARCODE,CREATE_NAME,CREATE_DATE,UPDATE_NAME,UPDATE_DATE,REMARK from PUB_ITEM_DA_BOM ");
			strSql.Append(" where ");
			SqlParameter[] parameters = {
			};

			MyERP.Model.PUB_ITEM_DA_BOM model=new MyERP.Model.PUB_ITEM_DA_BOM();
		...
		/// 得到一个对象实体
		/// </summary>
		public MyERP.Model.PUB_ITEM_DA_BOM DataRowToModel(DataRow row)
		{
			MyERP.Model.PUB_ITEM_DA_BOM model=new MyERP.Model.PUB_ITEM_DA_BOM();
			if (row != null)
			{
		...
		public DataSet GetList(string strWhere)
		{
			...
			strSql.Append(" FROM PUB_ITEM_DA_BOM ");
			if(strWhere.Trim()!="")
			{
		...
		{
			StringBuilder strSql=new StringBuilder();
			strSql.Append("select count(1) FROM PUB_ITEM_DA_BOM ");
			if(strWhere.Trim()!="")
			{
				strSql.Append(" where "+strWhere);
			}
		...
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			...
			strSql.Append(")AS Row, T.*  from PUB_ITEM_DA_BOM T ");
		...
		public DataSet GetList(int PageSize,int PageIndex,string strWhere)
		{
			...
			parameters[0].Value = "PUB_ITEM_DA_BOM";
